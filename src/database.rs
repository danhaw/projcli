...
macro_rules!  create_tables {
    ...
    ($($table_name:ident => {
        $($column:ident:$value_type:pat$(,)?)*
         ...
         //this vector will hold each "create table statment" that user provided and it will be returned at the end of the macro
        ...
        //this pattern will repeat depending on the number of the inner repeation provided
        //in the macro of the columns and thier types
        ...
        //constracting the sql create statement it's really simple for now
        ...
        //I have two better options to deal with the wrong types either
        //by returning None and let the user handle it or by panicing
        ...
        //that's for now, at least until I provide an implementaion for
        //the macro where I have to deal with it
        let valid_columns:Vec<String> = types.into_iter().map(|v_type|  {
            ...
            //TODO: change the return of none captured type
            match captures.as_ref().map(|c| c.as_slice()) {
                //returning the converted type directly to
                //the lambda function provided for the map to replace the current user provided type
                ...
                Some(["reference", referenced_table_name]) => {
                    //in case of foreign key it has a reference to another table
    ...
    )+ //notice pattern repeatition for each table ends here
